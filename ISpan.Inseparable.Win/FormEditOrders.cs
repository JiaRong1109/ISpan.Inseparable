...
{
	public partial class FormEditOrders : Form
	{
		...
		public FormEditOrders(int orderId)
		{
			InitializeComponent();
			this.orderId = orderId;
			repo= new OrderRepository();

			this.Load += FormEditOrders_Load;
		}
		public OrderUpdateVm GetModel()
			=> new OrderUpdateVm
			{
				OrderID = orderId,
		...
		private void FormEditOrders_Load(object sender, EventArgs e)
		{
		...
		private void BindForm(Orders get)
		{
			...
			comboBoxCinema.Text = get.Cinemas.CinemaName;
			labelOrderDate.Text = get.OrderDate.ToString();
			labelModifiedDate.Text = DateTime.Now.ToString();
		}
		private (bool isValid, List<ValidationResult> errors) Validate(OrderUpdateVm vm)
		{
			// 得知要驗證規則
